...
Campus 是面向学校教师的智能讲评系统，聚焦于从试卷和成绩表自动生成学情诊断、推荐练习与 Word 讲评报告，致力于提升考试讲评和课后巩固效率。
...
- 教师端首页与导航优化：工作区布局更紧凑，导航固定展示当前流程与矫正状态。
- 知识点智能矫正升级：默认处理全卷题目，支持批量模型请求，并同步刷新诊断与推荐题。
- 题目核对与推荐题流程完善：题干、选项、题图确认更清晰，推荐题可编辑。
- 代码结构整理：顶层 P2 包统一为 `campus_p2/`，移除无主流程引用的 `qpaper/`。
...
如需启用大模型能力，请在本地 `.env` 中配置 `CAMPUS_LLM_BASE_URL`、`CAMPUS_LLM_API_KEY` 和 `CAMPUS_LLM_MODEL`。`CAMPUS_LLM_TAG_BATCH_SIZE` 控制每批矫正题量。仓库只提交 `.env.example`，不要提交真实密钥。
...
- P3：题库、知识点、推荐题和训练包接口。
- 大模型：可选 OpenAI-compatible API，用于全卷知识点智能矫正和题目生成；未配置时使用规则兜底。
