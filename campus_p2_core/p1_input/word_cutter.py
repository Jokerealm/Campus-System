...
TRAILING_EXAM_TITLE_RE = re.compile(
    r"^(?:\d{4}年)?[\u4e00-\u9fff]{0,32}(?:中考|初中学业水平考试|学业水平考试|高考)?"
    r"[\u4e00-\u9fff]{0,24}数学(?:试卷|试题|真题|试题卷|考试卷)$"
)
...
def cut_docx_to_paper(
    ...
    *,
    ...
    try:
        ...
        _trim_last_question_trailing_artifacts(drafts, _paper_name_from_path(source_path))
...
def _trim_last_question_trailing_artifacts(drafts: list[QuestionDraft], paper_name: str) -> None:
    if not drafts:
        return

    last = drafts[-1]
    while last.blocks and _is_standalone_trailing_artifact(last.blocks[-1], paper_name):
        last.blocks.pop()
    ...
    block = last.blocks[-1]
    text, text_removed = _trim_trailing_artifact_lines(block.text, paper_name)
    markdown, markdown_removed = _trim_trailing_artifact_lines(block.markdown, paper_name)
    if text_removed or markdown_removed:
        block.text = text
        block.markdown = markdown or text


def _is_standalone_trailing_artifact(block: Block, paper_name: str) -> bool:
    if block.image_refs or block.formula_count:
        return False
    text = block.markdown or block.text
    return _looks_like_trailing_artifact(text, paper_name)


def _trim_trailing_artifact_lines(value: str, paper_name: str) -> tuple[str, int]:
    lines = value.splitlines()
    removed = 0
    while lines and _looks_like_trailing_artifact(lines[-1], paper_name):
        lines.pop()
        removed += 1
    return "\n".join(lines).strip(), removed


def _looks_like_trailing_artifact(value: str, paper_name: str) -> bool:
    compact = _normalize_title_text(value)
    if not compact:
        return False

    normalized_name = _normalize_title_text(paper_name)
    if normalized_name and normalized_name not in {"paper", "试卷"} and compact == normalized_name:
        return True

    if len(compact) <= 64 and TRAILING_EXAM_TITLE_RE.fullmatch(compact):
        return True

    if len(compact) <= 120 and any(marker in compact for marker in ("声明", "版权", "来源", "本试卷")):
        return True
...
def _normalize_title_text(value: str) -> str:
    text = IMAGE_PLACEHOLDER_RE.sub("", value)
    text = re.sub(r"!\[[^\]]*\]\([^)]+\)", "", text)
...
def _build_question_payload(
    ...
    stem_text = _strip_embedded_image_text_markers(_replace_image_placeholders(stem_text, {})) if exported_images else _replace_image_placeholders(stem_text, {})
...
def _strip_embedded_image_text_markers(value: str) -> str:
    text = re.sub(r"\s*\[(?:图|图片)\s*[0-9一二三四五六七八九十]+\]\s*", "\n", value)
    text = re.sub(r"\n{3,}", "\n\n", text)
    return text.strip()
